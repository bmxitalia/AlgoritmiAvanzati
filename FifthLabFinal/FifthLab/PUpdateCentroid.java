...
public class PUpdateCentroid extends RecursiveAction {
	...
	protected void compute() {
		if((end - start) <= cutoff) {
			...
			invokeAll(new PUpdateCentroid(cutoff, points, cluster, u, start, (int)Math.floor((start + end)/2)),
				new PUpdateCentroid(cutoff, points, cluster, u, (int)Math.floor((start + end)/2) + 1, end));
		}
	}
}
...
public class PReduceCluster extends RecursiveTask<SumSize> {
	...
	protected SumSize compute() {
		if((end - start) <= cutoff) {
			...
			PReduceCluster p1 = new PReduceCluster(cutoff, p, cluster, start, (int)Math.floor((start + end)/2), h);
			p1.fork();
			PReduceCluster p2 = new PReduceCluster(cutoff, p, cluster, (int)Math.floor((start + end)/2) + 1, end, h);
